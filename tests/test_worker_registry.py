...
def test_gemini_is_setup_required_candidate_with_no_authorised_role():
    gemini = get_worker_profile("gemini")
    assert gemini.approval_state == WorkerApprovalState.CANDIDATE
    assert not gemini.launchable
    assert gemini.requires_owner_setup
    assert gemini.authorised_roles == ()
    assert all(gemini not in eligible_workers(role) for role in WorkerRole)
...
def test_approved_roles_are_bounded_and_deterministic():
    assert [item.name for item in eligible_workers("implementation")] == [
        "kimi", "kimi-swarm", "codex"
    ]
    assert [item.name for item in eligible_workers("review")] == ["kimi-swarm"]
    assert [item.name for item in eligible_workers("fallback")] == ["codex"]
