...
def test_empty_summary_is_all_zero():
    summary = DashboardService(FakeRepository(), FakeRepository()).get_summary()
    assert summary == DashboardSummary(0, 0, 0, 0, 0, 0, 0)
...
def test_populated_summary_counts_known_and_other_statuses():
    ...
    summary = DashboardService(projects, knowledge).get_summary()
    ...
    assert projects.list_calls == knowledge.list_calls == 1
