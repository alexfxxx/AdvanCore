...
def _evidence():
    ...
    verification = ManifestVerificationEvidence(
        task_id="TASK-148",
        allowed_paths=PATHS,
        workspace_branch="task-148-swarm",
        verified_at=NOW,
        verification_id="verification-0001",
    )
    return queue, reservation, workspace, verification


def _evaluate(**overrides):
    queue, reservation, workspace, verification = _evidence()
    values = {
        "task_id": "TASK-148",
...
def test_explicit_architecture_work_can_have_small_scope():
    ...
    verification = replace(verification, allowed_paths=paths)
    result = _evaluate(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"manifest_verification": None}, SwarmEligibilityReason.MANIFEST_NOT_VERIFIED),
        (
...
def test_workspace_must_be_ready():
    ...
    assert _evaluate(
        workspace=replace(workspace, branch="task-999-unrelated")
    ).reason == SwarmEligibilityReason.WORKSPACE_NOT_READY


def test_small_multifile_scope_is_unsuitable_when_other_evidence_matches():
    paths = tuple(f"a{index}.py" for index in range(10))
    _, reservation, _, verification = _evidence()
    result = _evaluate(
        allowed_paths=paths,
        reservation=replace(reservation, paths=paths),
        manifest_verification=replace(verification, allowed_paths=paths),
    )
...
def test_gate_does_not_mutate_input_evidence():
    queue, reservation, workspace, verification = _evidence()
    before = (queue, reservation, workspace, verification)
    _evaluate(
        queue_record=queue,
        reservation=reservation,
        workspace=workspace,
    ...
    assert (queue, reservation, workspace, verification) == before


def test_manifest_verification_is_bound_to_task_scope_and_workspace():
    _, _, _, verification = _evidence()
    assert _evaluate(
        manifest_verification=replace(verification, task_id="TASK-999")
    ).reason == SwarmEligibilityReason.MANIFEST_NOT_VERIFIED
    assert _evaluate(
        manifest_verification=replace(verification, allowed_paths=("wrong.py",))
    ).reason == SwarmEligibilityReason.MANIFEST_NOT_VERIFIED
    assert _evaluate(
        manifest_verification=replace(
            verification, workspace_branch="task-148-other"
        )
    ).reason == SwarmEligibilityReason.MANIFEST_NOT_VERIFIED
    assert _evaluate(
        manifest_verification=replace(verification, verification_id="short")
    ).reason == SwarmEligibilityReason.MANIFEST_NOT_VERIFIED


def test_stale_claim_and_overlong_reservation_fail_closed():
    queue, reservation, _, _ = _evidence()
    stale_queue = replace(
        queue,
        enqueued_at=NOW - timedelta(hours=4),
        claimed_at=NOW - timedelta(hours=3),
    )
    assert _evaluate(queue_record=stale_queue).reason == (
        SwarmEligibilityReason.TIME_INVALID
    )
    assert _evaluate(
        reservation=replace(
            reservation,
...
def test_malformed_typed_evidence_returns_a_bounded_reason():
    queue, reservation, _, _ = _evidence()
    assert _evaluate(queue_record=replace(queue, task_path=None)).reason == (
        SwarmEligibilityReason.QUEUE_MISMATCH
    )
